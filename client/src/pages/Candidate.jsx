...
const Candidate = ({ state, handleCase }) => {
  ...
  useEffect(() => {
    if (state && state.contract && state.signer) setWalletConnected(true);
  }, [state]);

  const handleInfo = async (event) => {
    event.preventDefault();
    if (!walletConnected || !state.contract) {
      toast.error("Please connect wallet first");
      return;
    }

    const candidatesInfo = {
      name: handleCase(name),
      party: party.toUpperCase(),
      age,
      gender: handleCase(gender),
    };

    try {
      await state.contract.candidateRegister(
        candidatesInfo.name,
      ...
      setName(""); setParty(""); setAge(""); setGender("");
    } catch (error) {
      console.error(error);
        ...
        <div className="flex flex-col justify-center items-center mt-[10%]">
          ...
          <form className="grid grid-rows-2 grid-flow-col gap-7" onSubmit={handleInfo}>
            <div className="grid grid-rows-4 grid-flow-col gap-10">
              {[
                { placeholder: "Enter name", value: name, setValue: setName },
                { placeholder: "Enter party", value: party, setValue: setParty },
                { placeholder: "Enter age", value: age, setValue: setAge },
                { placeholder: "Enter gender", value: gender, setValue: setGender },
              ].map((field, index) => (
                <div
                  key={index}
                  className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50"
                >
                  <input
...
//   // Watch state.contract changes
//   useEffect(() => {
//     if (state && state.contract) setWalletConnected(true);
//   }, [state]);
...
//                     <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200  h-10" placeholder="Enter name " name="" id="name" value={name} onChange={(e)=>setName(e.target.value)} ></input>
//                 </div>

//                 <div className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50 ">
//                     <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200  h-10"placeholder="Enter party" name="" id="party" value={party} onChange={(e)=>setParty(e.target.value)} ></input>
//                 </div>

//                 <div className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50 ">
//                     <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200  h-10" placeholder="Enter age" name="" id="age" value={age} onChange={(e)=>setAge(e.target.value)}></input>
//                 </div>

//                 <div className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50 ">
//                     <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200  h-10" placeholder="Enter gender" name="" id="gender" value={gender} onChange={(e)=>setGender(e.target.value)}></input>
//                 </div>
//               </div>
