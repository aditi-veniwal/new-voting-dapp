...
const ElectionCommission = ({ state }) => {
  ...
  const handleElection = async (event) => {
    ...
    try {
      const tx = await state.contract.voteTime(start, end); // ethers v6 call
      await tx.wait();
      toast.success("Voting has been initialised", {
        description: `${date.toDateString()} at ${date.toLocaleTimeString()}`
      });
    } catch (error) {
      console.error(error);
          ...
          <form className="grid grid-rows-2 grid-flow-col gap-10" onSubmit={handleElection}>
            <div className="grid grid-rows-2 grid-flow-col gap-10">
              <div className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50 ">
                <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200 h-10" placeholder="Start time in seconds" id="startT" />
              </div>

              <div className="w-[100%] col-span-10 rounded-3xl p-px bg-gradient-to-b from-blue-300 to-pink-300 dark:from-blue-800 dark:to-purple-800 transition-all duration-700 hover:shadow-[0_3px_10px_rgb(0.4,0.4,0.4,0.4)] dark:hover:shadow-cyan-500/50 ">
                <input className="rounded-[calc(1.5rem-1px)] p-6 w-[100%] bg-white dark:bg-gray-900 dark:text-slate-200 h-10" placeholder="End time in seconds" id="endT" />
              </div>
            </div>
