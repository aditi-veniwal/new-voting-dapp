...
function App() {
  const [state, setState] = useState({
    provider: null,
    contract: null,
    signer: null, // store signer object, not just address
    signerAddress: null, // store address separately
  });

  const [info, setInfo] = useState();
  const [pIdEc, setPIdEc] = useState({ pollId: null, EcAddress: null });

  const wallet = (provider, contract, signer, signerAddress) => {
  ...
  // Only check login if user navigates to protected pages
  const checkLogin = () => {
  ...
  // Debug: expose React state globally
  useEffect(() => {
    window.state = state;
  }, [state]);
          ...
          <Route
            ...
            element={
              <Dashboard
                state={state}
                info={info}
                details={(pollId, EcAddress) =>
                  setPIdEc({ pollId, EcAddress })
                }
                setinfo={setInfo}
                pIdEc={pIdEc}
